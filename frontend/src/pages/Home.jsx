...
const Home = () => {
  ...
  const { onlineUser, useAuth } = authState();
  ...
  useEffect(() => {
    if (selectedUser) {
      console.log("Selected user:", selectedUser.username); // Debug log
      messageWindowState();
      mess();
    }
    
    // Cleanup when selectedUser changes
    return () => {
      if (selectedUser) {
        console.log("Cleaning up for user:", selectedUser.username); // Debug log
        submess();
      }
    };
  }, [selectedUser?._id]); // Only depend on the ID to avoid unnecessary re-runs
